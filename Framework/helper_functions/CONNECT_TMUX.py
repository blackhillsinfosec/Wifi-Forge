...
def CONFIG_TMUX(nodes, lab_name):
    try:
        ...
        for index, node in enumerate(nodes):
            ...
            process_id = check_output(["ps aux | grep -G 'mininet:"+node+"' | grep -v 'grep' | grep -v 'ap1' | awk '{print $2}'"], shell=True).decode("utf-8")
            subprocess.Popen(["tmux", "send-keys", "-t", f"{session_name}:0.{index}", f"exec sudo nsenter -t {process_id.rstrip()} -m -u -i -n -p bash; clear", "C-m"], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
